...
from aoms.contracts import MemoryKind, MemoryRecord, Provenance, RecallRequest
...
async def test_export_restore_round_trip_includes_receipts_and_manifest_hashes(
    ...
    engine = RecallEngine(source, embedding_provider=NullProvider())
    ...
    assert len(await target.recent_recall_receipts()) == 1
