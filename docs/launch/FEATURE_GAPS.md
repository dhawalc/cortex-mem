...
| 60: decides whether to keep it | Export/restore is hash-verified, restore refuses non-empty targets, `doctor` is credible, local storage claims are precise, and relay rehearsal evidence exists. | The user has seen almost none of this. There is no personal outcome (“three handoffs, 842 recalled tokens, two stale facts suppressed”), visual memory surface, or shareable evidence card. | Close the hour with evidence sourced from receipts—not invented “tokens saved.” Visibility is the viral loop for an otherwise invisible daemon. |
